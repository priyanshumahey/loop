...
export default function PromptBar({
  ...
  const productionMode = controlled || onSubmit !== undefined;
  const [internalDraft, setInternalDraft] = useState("");
  const draft = value ?? internalDraft;
  const setDraft = (next: SetStateAction<string>) => {
    const nextDraft = typeof next === "function" ? next(draft) : next;
    if (!controlled) setInternalDraft(nextDraft);
  ...
  const [auto, setAuto] = useState(!productionMode);
  ...
  const applyAutoStep = useEffectEvent((step: (typeof AUTO_STEPS)[number]) => {
    setDraft(step.draft);
    ...
    if (step.modelOpen !== undefined) {
      ...
      if (!step.modelOpen) setModelHovered(null);
    }
    if (step.model) {
  ...
  useEffect(() => {
    ...
    const applyTimer = window.setTimeout(() => applyAutoStep(step), 0);
  ...
  const finishDictation = useEffectEvent(() => {
  ...
  useEffect(() => {
    if (!listening) return;
    const timer = window.setTimeout(finishDictation, 2200);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {footerLeading && <div className="px-0.5 pt-0.5">{footerLeading}</div>}
      </div>
      </div>
    </div>
  );
